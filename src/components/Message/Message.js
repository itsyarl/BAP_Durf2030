  ...
  return useObserver(() => (
    <>
      {message.messages.map(messageObj => ( 
        ...
        <img className={`${style.img} ${uiStore.currentUser.name === messageObj.user.name ? style.img__me : ""}`} src={user} alt="user foto"/> 
          <div className={`${style.content} ${uiStore.currentUser.name === messageObj.user.name ? style.content__me : ""}`}>
            <p className={`${style.user} ${uiStore.currentUser.name === messageObj.user.name ? style.user__me : ""}`}>username <span className={style.user__rol}>rol</span>:</p>
